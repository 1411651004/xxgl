...
layui.config({
	base : '../admin/js/lib/' //layui自定义layui组件目录
}).extend({
	larry : 'larry',
	navtab : 'navtab',
	elemnts : 'elements',
...
layui.use([ 'elements', 'jquery', 'layer', 'larry', 'navtab', 'form', 'common' ], function() {
	...
	$(document).ready(function() {
		...
		$.getJSON('../admin/datas/top_menu.json?t=' + new Date(), {
		    Param: 'index_menu'
		}, function (result) {
		    larry.set({
		        elem: '#menu',
		...
		$menu.find('li.layui-nav-item').each(function() {
			...
			$that.on('click', function() {
				...
				$.getJSON("http://" + window.location.host + '/login/loadAuth', function(result) {
					//alert(result);
					pid_0 = eval(result);
				});
	...
	form.on('select(jd)', function(data){
...
//	    console.log(data.othis); //得到美化后的DOM对象
	...
	var jdManageIndex = 0;
	form.on('submit(submitlocal1)', function(data) {
		if(data.elem.id == "LarryThemeSetCancel") {
			// 取消
			layer.close(jdManageIndex);
		} else if(data.elem.id == "LarryThemeSetSubmit") {
			// 提交
			$.ajax({
				url:"/xtgl/setQuarter",
				data : {
					id:$("#jd").val(),
					codevalue:$("#jd").find("option:selected").text()
				},
	...
	$('#larryTheme').on('click', function() {
		...
		$.ajax({
			...
			success : function(data) {
				var opts = "<option value=\"\">请选择季度</option>";
				if (data && data.length > 0) {
					var jdId = $("#jdId").val();
					for (var i = 0; i < data.length; i++) {
						opts += "<option value=\"" + data[i].id + "\">" + data[i].codevalue + "</option>";
					}
					$("#jd").html(opts);
					$("#jd").find("option[value='"+jdId+"']").attr("selected",true); 
					form.render();
				}
				//        var fScreen = localStorage.getItem('fullscreen_info');
				var themeName = localStorage.getItem('themeName');
				jdManageIndex = layer.open({
					type : 1,
					title : false,
					closeBtn : true,
					shadeClose : false,
	...
	var jdIdTemp = $("#jdId").val();
	if(!jdIdTemp || jdIdTemp == "null") {
		// 季度选择
		$('#larryTheme').click();
	}
	// 全屏切换
