    ...
    <div class="layui-inline" id="search">
    	<div style="width:auto;" class="layui-input-inline">
      		<button class="layui-btn" lay-submit=""  lay-filter="formDemo">查询</button>
	      		<a class="layui-btn" onclick="newqyxx()">新增</a>
	      		<a class="layui-btn" onclick="sendmessage()">批量发送邮件</a>
	      		<c:if test="${record.type == '0'}">
	      		    <a class="layui-btn" onclick="exportQyxx()">导出相关文件</a>
	      		</c:if>
      	</div>
    </div>
  </div>
...
function newqyxx(){
  	window.location.href="<%=basePath %>qyxx/add?type=${record.type}";
  }
function upd(id){
  	window.location.href="<%=basePath %>qyxx/add?type=${record.type}&id="+id;
  }
/**
 * 访问记录
 */
function fwjl(id){
    window.location.href="<%=basePath %>qyxx/fwjl?type=${record.type}&id="+id;
}
function del(id){
	layui.use(['jquery','layer'], function(){
	 ...
	 $.ajax({
			url:"<%=basePath %>qyxx/delete?id="+id,
			dataType:"json",
			type:"post",
			 beforeSend:function(){
		         layer.msg('请稍候', {
...
function exportQyxx(){
	layui.use(['form','laydate','upload','element','jquery','layer','layedit'], function(){
		 ...
		 $.ajax({
			 ...
			 dataType:"json",
			 success :function(data){
				 if(data && data.code==0 && data.fileName) {
					 window.open("<%=basePath%>uploadFile/zip/"+data.fileName);
				 } else {
					 alert("导出失败");
				 }
			 }
...
layui.use(['form','laydate','upload','element','jquery','layer'], function(){
 ...
 form.on('checkbox(ckall)', function(data){
		if(data.elem.checked){
			...
			$("[group='ids']").each(function(){
				...
				primarys.push($(this).attr("primarys"));
 ...
 form.on('checkbox(ckone)', function(data){
	 if(data.elem.checked){
		...
		primarys.push($(data.elem).attr("primarys"))
	 	phonesarr.push($(data.elem).val());
	 }else{
		 removeByValue(idsarr,$(data.elem).attr("idnum"));
		 removeByValue(primarys,$(data.elem).attr("primarys"));
